...
def iniciar_sesion(correo, contra):
    ...
    h = h.hexdigest()
    print(h)
    query = "SELECT id_usuario FROM usuario WHERE correo = %s AND contrasena = %s"
...
def insertar_peliculas(pelicula):
    ...
    anio = pelicula['anio']
    idusuario = pelicula['idusuario'] 

    insertar = "INSERT INTO pelicula \
        (titulo, fecha_visto, imagen, director, anio, idusuario) \
        VALUES (%s, %s, %s, %s, %s, %s)"
    cursor.execute(insertar, (titulo, fecha_visto, imagen, director, anio, idusuario))
    bd.commit()
...
def get_peliculas():
    query = "SELECT id_pelicula, titulo, imagen, fecha_visto, director, anio FROM pelicula"
...
def get_pelicula(id):
    query = "SELECT * FROM pelicula WHERE id_pelicula = %s"
    cursor.execute(query, (id,))
    pelicula = {}
    row = cursor.fetchone()
    if row:
        pelicula['id_pelicula'] = row[0]
...
def modificar_pelicula(id, columna, valor):
    update = f"UPDATE pelicula SET {columna} = %s WHERE id_pelicula = %s"
...
def eliminar_pelicula(id):
    eliminar = "DELETE from pelicula WHERE id_pelicula = %s"
...
def get_peliculas_usuario(id):
    query = "SELECT * FROM pelicula WHERE idusuario = %s"
    cursor.execute(query, (id,))
    peliculas = []
    for row in cursor.fetchall():
        pelicula = {
